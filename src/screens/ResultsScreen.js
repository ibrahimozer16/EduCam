...
export default function ResultsScreen({ navigation }) {
  ...
  const renderResult = ({ item }) => (
    <View style={styles.resultBox}>
      ...
      <Text style={styles.score}>Puan: {item.score} / {item.total}</Text>
      {item.feedback && <Text style={styles.feedback}>💬 {item.feedback}</Text>}
      <Text style={styles.date}>📅 {new Date(item.date?.toDate()).toLocaleString()}</Text>
    </View>
  );
    ...
    <View style={{ flex: 1, flexDirection: 'column' }}>
      <View style={{ flex: 1 }}>
        ...
        <FlatList
          ...
          ListEmptyComponent={<Text style={styles.empty}>Henüz sonuç yok</Text>}
...
const styles = StyleSheet.create({
  ...
  header: { flexDirection: 'row', gap: 10, alignItems: 'center', justifyContent: 'center',marginBottom: 10, },
  headerText: { fontSize: 40, color: '#1e3a8a'  },
  title: { fontSize: 20, fontWeight: 'bold', paddingVertical: 10 ,color: '#1e3a8a' },
  resultBox: {
